...
def cmd_decompile(
    input_file: Annotated[Path, typer.Argument(help="Input .binfnt file path")],
    output_dir: Annotated[Path | None, typer.Argument(help="Output directory")] = None,
...
def cmd_compile(
    original_file: Annotated[
        Path, typer.Argument(help="Path to original .binfnt file")
    ],
    modified_file: Annotated[Path, typer.Argument(help="Path to modified .fnt file")],
    output_file: Annotated[Path, typer.Argument(help="Output .binfnt file path")],
):
    with Progress(
        SpinnerColumn(finished_text="\u2713"),
        TextColumn("[progress.description]{task.description}"),
