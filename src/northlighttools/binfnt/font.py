...
class BinaryFont:
    ...
    def __load(self, file_path: Path):
        with file_path.open("rb") as reader:
            ...
            self.__read_kernel_block(reader)
    ...
    def __read_unknown_block(self, reader):
        ...
        for _ in range(len(self.__characters)):
            self.__unknown.append(Unknown(*unpack("6H", reader.read(12))))
    ...
    def __read_advance_block(self, reader):
        ...
        for _ in range(len(self.__characters)):
            self.__advance.append(Advance(*unpack("4HI8f", reader.read(44))))
    ...
    def __read_kernel_block(self, reader):
        self.__progress.console.log("Reading kernel block...")
        ...
        for _ in range(kerning_count):
            self.__kernel.append(Kernel(*unpack(fmt, reader.read(size))))
    ...
    def __calculate_font_properties(self):
        ...
        for idx, char in enumerate(self.__characters):
            ...
            line_height = (
                -self.__advance[idx].yoffset2_1 * size
                + point.height
        ...
        with font_path.open("w", encoding="utf-8") as f:
            font_data = {
                ...
                "characters": {
                    self.__id_table[char_id]: asdict(
                        char.to_character(
                            texture_width=self.__texture.width,
                            texture_height=self.__texture.height,
                            advance=self.__advance[char_id],
                            line_height=self.__line_height,
                            font_size=self.__font_size,
                        )
                    )
                ...
                "kernels": [
                    ...
                    for ker in self.__kernel
                ],
                "unknowns": [asdict(unk) for unk in self.__unknown],
    ...
    def from_json(self, json_path: Path, separate_characters: bool = False):
        ...
        self.__kernel = [
            Kernel(**ker).with_font_size(self.__font_size, self.__version)
            for ker in font_data["kernels"]
        ]
    ...
    def compile(self, output_path: Path):
        ...
        self.__progress.console.log("Writing font data to binary file...")
    ...
    def __write_kernel_block(self, writer):
        writer.write(len(self.__kernel).to_bytes(4, "little"))
